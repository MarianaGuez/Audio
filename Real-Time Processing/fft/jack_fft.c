...
double *freqs;
/**
 * La ventana de jack debe ser la mitad de la ventana de Hann. No desfasar tiempos mayores a la mitad de la ventana de jack
 *
 *
...
int jack_callback (jack_nframes_t nframes, void *arg){
  ...
  for(i = 0; i < nframes; i++){
    ...
    /*o_fft[i] = i_fft[i]*ce^(-I*2*pi*abs(freqs[i])*desfase_en_segundos);
     */
  }
